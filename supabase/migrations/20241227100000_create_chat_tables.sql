...
-- RLS Policies - Allow all access (auth handled at API layer with service role)
CREATE POLICY "Allow all Chat access" ON "Chat" FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Allow all ChatMessage access" ON "ChatMessage" FOR ALL USING (true) WITH CHECK (true);
