...
-- Step 1: Create a Chat for each WorkOrder that has a subcontractor assigned (via Job)
INSERT INTO "Chat" ("workOrderId", "organizationId", "subcontractorId", "createdAt", "updatedAt")
SELECT DISTINCT
    wo."id" as "workOrderId",
    wo."organizationId",
    j."subcontractorId" as "subcontractorId",
    wo."createdAt",
    NOW()
FROM "WorkOrder" wo
LEFT JOIN "Job" j ON j."id" = wo."jobId"
WHERE j."subcontractorId" IS NOT NULL
