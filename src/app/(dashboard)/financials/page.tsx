...
export default function FinancialsPage() {
  const [revenueTarget, setRevenueTarget] = useState(DEFAULT_VTO_SETTINGS.annualTarget);
  const [isLoading, setIsLoading] = useState(true);

  // Load VTO settings from API to get the revenue target
  useEffect(() => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.annualTarget) {
            setRevenueTarget(data.annualTarget);
          }
        }
      } catch (e) {
  ...
  // Target tracking - now uses VTO annualTarget
              ...
              <div>
                ...
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className="text-xs text-slate-500">
                    {revenueProgress.toFixed(0)}% of {formatCurrency(revenueTarget, true)} target
                  </span>
                </div>
              </div>
