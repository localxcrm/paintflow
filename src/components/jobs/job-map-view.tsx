...
export function JobMapView({ jobs, subcontractors }: JobMapViewProps) {
  ...
  }, [jobs, statusFilter, selectedSubcontractor, periodFilter]);

  // Transform jobs for map component (expecting Subcontractor with capital S)
  const mapJobs = useMemo(() => {
    return filteredJobs
      ...
      .map(job => ({
        ...
        status: job.status,
        scheduledStartDate: job.scheduledStartDate || null,
        latitude: job.latitude || null,
        longitude: job.longitude || null,
        jobValue: job.jobValue,
        Subcontractor: job.subcontractor ? {
          id: job.subcontractor.id,
          name: job.subcontractor.name,
          color: job.subcontractor.color || '#6b7280',
        } : null,
      }));
  }, [filteredJobs]);
